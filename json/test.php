...
for($i=0;$i<1;$i++){
    foreach($prov as $c => $d){
        $ch=curl_init("https://sirekap-obj-data.kpu.go.id/pemilu/hhcw/ppwp/$c.json");
        curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);curl_setopt($ch, CURLOPT_NOBODY, true);curl_setopt($ch, CURLOPT_HEADER, true);curl_setopt($ch, CURLOPT_FILETIME, true);curl_exec($ch);curl_close($ch);
        echo date("r",curl_getinfo($ch)["filetime"])." - $d\n";
